...
- [ ] 2.1 In `src/session/saver.js`, remove the `try { await stat(dir) } catch { await mkdir(...) }` block (lines 13-17)
- [ ] 2.2 Remove `mkdir` and `stat` from the import from `node:fs/promises`
- [ ] 2.3 Add `@throws {Error}` JSDoc to `saveSession()` documenting that unhandled filesystem errors propagate

## 3. Update shutdown handler to propagate fatal errors

- [ ] 3.1 In `src/session/shutdown.js`, modify `handleShutdown()` so that `saveSession()` errors are not caught — narrow the try/catch to only surround `flushTelemetry()`
...
- [ ] 4.1 In `index.js`, wrap the `onSaveSession` callback (line 223) in try/catch and call `process.exit(1)` on error
...
- [ ] 5.3 Add test: `saveSession` writes file successfully when directory exists (existing behavior)
- [ ] 5.4 Add test: `saveSession` propagates writeFile errors unhandled
