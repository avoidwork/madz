...
- Render results sorted by length (shortest first), then locale collation, with a rotating window (up to 3 visible), using the `▸` indicator matching `/skills` and `/sessions`.
