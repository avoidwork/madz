...
- [ ] 1.1 Run `npm install fast-glob` to promote `fast-glob` to a direct dependency in `package.json`
...
- [ ] 2.7 Debounce the initial glob at ~250ms on filter change
- [ ] 2.8 Render up to 3 visible options with a rotating window and the `▸` indicator (mirroring `ink-select-input`'s `limit` behavior)

## 3. Wire Into InputArea

- [ ] 3.1 Render `FilePicker` below the `InputPanel` in `src/tui/inputArea.js` when open
- [ ] 3.2 Set `focus={false}` on the `InputPanel` while the picker is open
- [ ] 3.3 Expose `isPickerOpen()` on the input ref via `useImperativeHandle`

## 4. App-Level Bail

- [ ] 4.1 In `src/tui/app.js`, in the conversation-view section of the global `useInput`, return early if `inputAreaRef.current?.isPickerOpen?.()` is true so up/down and Escape don't steal keys from the picker
