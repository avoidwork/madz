...
- [x] 1.3 Preserve the `autoContinueCountRef` / `config?.agent?.autoContinueLimit` loop guard so the reasoning-only continue is bounded
...
- [x] 2.2 Add a test asserting the auto-continue limit guard stops the loop
