...
- Do not change JSON file format in `memory/schedules/` (except adding `command` field).
- Do not force jobs to run via `madz --run-schedule`. `crond` executes the command directly.

## Decisions

### 1. CronInstaller renamed to Cron with CRUD ops
**Decision**: Keep the existing crontab block management logic. Add methods:
- `add({ name, cron, command, ... })` — reads crontab, removes existing madz block, appends new entry with exact command, writes crontab back.
- `remove(name)` — reads crontab, removes madz block, writes back everything except the named entry.
- `list()` — reads crontab madz block and returns array of `{ name, cron, command }`.
...
| Jobs without a `command` field | `Cron.add()` will validate that `command` is present before writing to crontab. |
