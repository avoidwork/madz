...
The TUI skill slash-command (`/skill-name`) is broken. When a user types a skill name as a slash command, it routes through a legacy IPC dispatch path that loads the SKILL.md body and feeds it to the model as a plain chat prompt, rather than invoking the skill through the deepagents skill system. The status bar also silently drops its status message, so the user gets no feedback that anything happened.
...
- Fix the status bar in `src/tui/statusBar.js` to render the `statusMessage` prop so the user sees feedback (e.g., "Ready", "Streaming...", skill execution status).
...
- `src/tui/statusBar.js` — render `statusMessage`.
