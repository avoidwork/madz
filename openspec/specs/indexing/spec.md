...
#### Scenario: Scanning runs off the main event loop

- **WHEN** the `indexCode` tool invokes the indexer
- **THEN** the scanning work is dispatched to a Piscina worker pool
- **AND** the main event loop is not blocked during scanning

### Requirement: Indexer tracks file mtimes

The system SHALL track the modification time (mtime) of each indexed file and skip files whose mtime has not changed since last index. The mtime tracking SHALL run in a Piscina worker pool.
