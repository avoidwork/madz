...
function tokenize(input) {
  ...
  while (pos < input.length) {
    ...
    if (ch === "+" || ch === "-" || ch === "*" || ch === "/" || ch === "^" || ch === "=" || ch === "<" || ch === ">") {
      ...
      pos++;
      // Handle !=, <=, >=
      if (pos < input.length && ((op === "!" && input[pos] === "=") || (op === "<" && input[pos] === "=") || (op === ">" && input[pos] === "="))) {
        op += input[pos];
        pos++;
      }
...
function parseAdd(tokens, ctx) {
  ...
  while (tokens[ctx.pos]?.type === TOKEN_TYPES.OPERATOR && ["+", "-"].includes(tokens[ctx.pos].value)) {
    const op = tokens[ctx.pos].value;
    ctx.pos++;
...
function parseUnary(tokens, ctx) {
  if (tokens[ctx.pos]?.type === TOKEN_TYPES.OPERATOR && ["-", "!"].includes(tokens[ctx.pos].value)) {
    const op = tokens[ctx.pos].value;
    ctx.pos++;
...
function parsePrimary(tokens, ctx) {
  ...
  // Boolean literals
  if (token?.type === TOKEN_TYPES.IDENTIFIER && ["TRUE", "FALSE"].includes(token.value.toUpperCase())) {
    ctx.pos++;
...
function evaluateNode(node, context, options) {
  ...
  switch (node.type) {
    ...
    case "unaryOp": {
      const operand = evaluateNode(node.operand, context, { ...options, depth: depth + 1, visited: new Set([...visited, ...getRefs(node.operand)]) });
    ...
    case "binaryOp": {
      const left = evaluateNode(node.left, context, { ...options, depth: depth + 1, visited: new Set([...visited, ...getRefs(node.left)]) });
      const right = evaluateNode(node.right, context, { ...options, depth: depth + 1, visited: new Set([...visited, ...getRefs(node.right)]) });

      switch (node.op) {
        case "+": return safeNumber(left) + safeNumber(right);
        case "-": return safeNumber(left) - safeNumber(right);
        case "*": return safeNumber(left) * safeNumber(right);
        case "/": {
        ...
        case "^": return Math.pow(safeNumber(left), safeNumber(right));
        ...
        case "<=": return safeNumber(left) <= safeNumber(right);
        case ">=": return safeNumber(left) >= safeNumber(right);
        case "&&": return evaluateCondition(left) && evaluateCondition(right);
        case "||": return evaluateCondition(left) || evaluateCondition(right);
        default: return left;
      }
    }
    ...
    case "functionCall": {
      ...
      const evaluatedArgs = node.args.map((arg) =>
        evaluateNode(arg, context, { ...options, depth: depth + 1, visited: new Set([...visited, ...getRefs(arg)]) })
      );
