    ...
    sheets: sheets.map((sheet) => {
      const rows = sheet.rows.map((row) => {
        ...
        row.values.forEach((value, i) => {
          const col = String.fromCharCode(65 + (i % 26));
          const cellRef = `${col}${row.values.indexOf(value) === i ? i + 1 : i + 1}`;
...
export async function modify(input) {
  ...
  for (const op of operations) {
    ...
    if (!sheet) {
      results.push({ operation: op.type, sheet: op.sheetName, status: "error", reason: `Sheet "${op.sheetName}" not found` });
      continue;
    }
    ...
    switch (op.type) {
      case "addCell": {
        ...
        results.push({ operation: op.type, sheet: op.sheetName, cell: op.cellRef, status: "added" });
        break;
      }
      case "modifyCell": {
        if (op.cellRef) {
        ...
        results.push({ operation: op.type, sheet: op.sheetName, cell: op.cellRef, status: "modified" });
        break;
      }
      case "deleteCell": {
        if (op.cellRef) {
        ...
        results.push({ operation: op.type, sheet: op.sheetName, cell: op.cellRef, status: "deleted" });
        break;
      }
      case "addSheet": {
        const newSheet = workbook.addWorksheet(op.sheetName);
      ...
      case "renameSheet": {
        ...
        results.push({ operation: op.type, sheet: op.sheetName, newName: op.newName, status: "renamed" });
        break;
      }
      default:
        results.push({ operation: op.type, sheet: op.sheetName, status: "error", reason: `Unknown operation: ${op.type}` });
    }
  }
...
export async function exportData(input) {
  ...
  switch (format) {
    ...
    case "xlsx": {
      ...
      await workbook.xlsx.writeFile(outputPath);
      return { format: "xlsx", status: "generated", outputPath, rows: data.length, columns: headers };
    }
    default:
...
export const spreadsheetModify = tool(modify, {
  ...
  description:
    "Modify an existing XLSX file: add/modify/delete cells, add/delete/rename sheets.",
  schema: ModifySchema,
});
