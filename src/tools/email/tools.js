 ...
 */
export const emailRead = tool(async ({ folder, limit, sender, subject, keyword, dateFrom, dateTo, label }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured. Set up email credentials in config.yaml." };
	}

	const validation = validateProviderConfig(config.email?.provider);
	if (!validation.valid) {
		return { ok: false, error: `Invalid email provider config: ${validation.errors?.join("; ")}` };
	}
	...
	name: "email_read",
	description: "Read emails from inbox, sent, drafts, or custom folders. Supports filtering by sender, subject, keyword, date range, and label.",
	schema: z.object({
		folder: z.string().optional().describe("Mailbox folder (default: INBOX). Examples: INBOX, Sent, Drafts, [Gmail]/Trash"),
 ...
 */
export const emailSend = tool(async ({ to, subject, body, bodyType, cc, bcc, attachments }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured. Set up email credentials in config.yaml." };
	}
	...
	name: "email_send",
	description: "Send an email with text/HTML body, attachments, CC/BCC support. Requires an email provider to be configured.",
	schema: z.object({
		to: z.array(z.string()).min(1).describe("Recipient email addresses"),
		subject: z.string().min(1).describe("Email subject line"),
		body: z.string().min(1).describe("Email body content"),
		bodyType: z.enum(["text", "html"]).optional().default("text").describe("Body format (default: text)"),
 ...
 */
export const emailDraftSave = tool(async ({ to, subject, body, bodyType }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
 ...
 */
export const emailDraftList = tool(async ({ limit }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured." };
	}
 ...
 */
export const emailDraftUpdate = tool(async ({ draftId, to, subject, body, bodyType }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured." };
	}
 ...
 */
export const emailDraftDelete = tool(async ({ draftId }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured." };
	}
 ...
 */
export const emailOrganize = tool(async ({ messageIds, action, label }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured." };
	}
	...
	if (!action) {
		return { ok: false, error: "Action is required (markRead, markUnread, archive, addLabel, removeLabel)" };
	}
	...
	name: "email_organize",
	description: "Organize emails: mark as read/unread, archive, add/remove labels. Requires message IDs.",
	schema: z.object({
		messageIds: z.union([z.string(), z.array(z.string())]).describe("Message ID or array of message IDs"),
		action: z.enum(["markRead", "markUnread", "archive", "addLabel", "removeLabel"]).describe("Organization action"),
 ...
 */
export const emailSearch = tool(async ({ query, limit }) => {
	const provider = getActiveProvider(config);
	if (!provider) {
		return { ok: false, error: "No email provider configured." };
	}
