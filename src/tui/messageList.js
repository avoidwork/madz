/**
 * MessageList component — manages a Map of MessageBubble component instances.
 * Provides imperative methods (addMessage, updateMessage, clear) and handles
 * scroll-to-bottom internally via its own ref.
 *
 * Using a Map (keyed by bubble ID) instead of an array ensures that
 * imperatively-added streaming bubbles survive the sync from the
 * messages prop, which is the source of truth for persisted messages.
 *
 * @module messageList
 */
...
export const MessageList = forwardRef(function MessageList({ scrollRef: externalScrollRef, height, messages }, ref) {
	...
	// Sync persisted messages from prop into the bubble Map.
	// Uses index-based IDs (msg-0, msg-1, ...) as the key so the
	// sync can find and update existing entries without wiping
	// imperatively-added streaming bubbles.
	useEffect(() => {
		...
		setBubbles((prev) => {
			...
			messages.forEach((msg, idx) => {
				...
				if (existing) {
					// Update existing synced bubble in place
					next.set(key, { ...existing, ...msg });
				} else {
					// New synced message — add it
					next.set(key, {
						id: key,
						role: msg.role,
						content: msg.content || "",
			...
			logger.info({ bubbleCount: next.size }, "[MessageList] bubbles synced");
			return next;
		});
	}, [messages?.length]);
	...
	useEffect(() => {
		...
		const bubbleArr = Array.from(bubbles.values());
		const lastBubble = bubbleArr[bubbleArr.length - 1];
		const isStreaming = lastBubble?.streaming === true;
		const streamingContentLen = isStreaming ? (lastBubble?.content || "").length : 0;
		const contentHash = bubbleArr.length + streamingContentLen;

		const wasScrolling =
			bubbleArr.length > previousMessageCount.current ||
		...
		const scrollHandle = () => {
			if (scrollRef.current) {
				...
				previousMessageCount.current = bubbleArr.length;
	...
	const bubbleArr = Array.from(bubbles.values());
	const startIdx = Math.max(0, bubbleArr.length - MAX_RENDER_MESSAGES);
	const visibleBubbles = bubbleArr.slice(startIdx);
