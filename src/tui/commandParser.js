...
export class CommandParser {
  ...
  constructor() {
    ...
    this.#register("quit", (args, ctx) => {
      return { action: "quit", value: true, message: ctx ? "Shutting down..." : "Quitting." };
    });
    ...
    this.#register("provider", (args, ctx) => {
      ...
      return { action: "provider", message: `Current provider: ${ctx._sessionState.getProvider()}` };
    });
    ...
    this.#register("memory", (args, ctx) => {
      if (args[0] === "open") {
        return { action: "memory", subAction: "open", message: ctx._contextList ? "Opening memory..." : "No memory entries." };
      }
    ...
    this.#register("context", (args, ctx) => {
      if (args[0] === "add") {
  ...
  parse(input, context) {
    ...
    if (!handler) {
      return { action: "unknown", message: `Unknown command: :${commandName}. Type :help for available commands.` };
    }
