/**
 * StatusBar — Bottom status bar with indicator, metrics, and toggle display.
 */
...
/**
 * Get connection status indicator and color.
 * @param {string} status
...
/**
 * Get toggle indicator string for status bar.
 * @param {{autoScroll: boolean, timestamps: boolean, commandEcho: boolean, cursorBreathe: boolean, debugOutput: boolean}} toggles
 * @returns {string}
 */
function getToggleIndicators(toggles) {
	if (!toggles) return "";
	const parts = [];
 ...
 * @param {number} props.messageCount - Total messages in conversation
 * @param {number} props.contextSize - Current conversation token count
 * @param {boolean} props.isCompacting - Whether context compaction is in progress
 * @param {Object} [props.toggles] - Runtime toggle states
 */
