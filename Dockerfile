...
RUN apk update && \
    ...
    printf '%s\n' '#!/bin/sh' '[ -f /etc/profile.d/madz-env.sh ] && . /etc/profile.d/madz-env.sh' 'if [ -x "/app" ]; then' '    echo "Starting madz..."' '    cd /app' '    if [ "${NODE_EXPOSE_GC}" = "1" ]; then' '        exec node --expose-gc index.js --mode interactive' '    fi' '    exec npm start' 'fi' > /etc/profile && \
    passwd -d madz && \
