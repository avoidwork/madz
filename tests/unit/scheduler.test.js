...
describe("scheduler - cron validation", () => {
  ...
  it("rejects invalid expression (too few fields)", () => {
    assert.deepStrictEqual(validateCron("* *"), { valid: false, error: "Expected 5-6 fields, got 2" });
  });

  it("rejects invalid expression (too many fields)", () => {
    assert.deepStrictEqual(validateCron("* * * * * * * *"), { valid: false, error: "Expected 5-6 fields, got 8" });
  });

  it("rejects null expression", () => {
    assert.deepStrictEqual(validateCron(null), { valid: false, error: "Cron expression is required" });
  });

  it("rejects empty expression", () => {
    assert.deepStrictEqual(validateCron(""), { valid: false, error: "Cron expression is required" });
  });
});
