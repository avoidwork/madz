...
describe("calendarImpl", () => {
	...
	it("should return error for read without startDate", async () => {
		...
		assert.ok(
			result.error.includes("Invalid calendar provider config") ||
				result.error.includes("startDate"),
		);
	});
	...
	it("should return error for create without title", async () => {
		...
		assert.strictEqual(result.ok, false);
		assert.ok(
			result.error.includes("Invalid calendar provider config") || result.error.includes("title"),
		);
	});
	...
	it("should return error for create without start/end", async () => {
		...
		assert.ok(
			result.error.includes("Invalid calendar provider config") ||
				result.error.includes("start and end"),
		);
	});
	...
	it("should return error for update without eventId", async () => {
		...
		assert.strictEqual(result.ok, false);
		assert.ok(
			result.error.includes("Invalid calendar provider config") || result.error.includes("eventId"),
		);
	});
