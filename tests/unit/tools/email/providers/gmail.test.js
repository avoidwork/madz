...
describe("GmailProvider", () => {
	...
	test("should throw when clientId is missing", () => {
		assert.throws(() => new GmailProvider({ clientSecret: "secret", refreshToken: "token" }), /clientId/);
	});

	test("should throw when clientSecret is missing", () => {
		assert.throws(() => new GmailProvider({ clientId: "id", refreshToken: "token" }), /clientSecret/);
	});
	...
	test("read() should return { ok: false } when googleapis is not installed", async () => {
		const provider = new GmailProvider({ clientId: "id", clientSecret: "secret", refreshToken: "token" });
