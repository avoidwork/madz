...
### CAPABILITIES
Ask the user: `clarify`. Compact context when needed: `compactContext`. Time awareness: `date`. Read and write memory: `memory`. Spawn processes: `process`. Scan project constraint files: `scanAgents`. Run shell commands: `shell`. Analyze images/schemas: `analyzeVision`. Search the web: `searchWeb`. Extract web content: `extractWeb`.
